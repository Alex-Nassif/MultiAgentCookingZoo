...
class CookingEnv:
    def __init__(self, **kwargs):
        ...
        self.env = parallel_env(
            ...
            action_scheme=kwargs.get("action_scheme", "scheme1"),
    ...
    def reset(self):
        self.t = 0
        self.frames = []  # <-- Add this line to clear frames for new episode
        obs = self.env.reset()  # Reset the environment to get initial observations
        if isinstance(obs, tuple):
            obs = obs[0]
        self.last_obs = obs
...
#        actions, comms = zip(*actions_with_comm)  # actions, communication bits
        ...
        for i, agent in enumerate(self.agents):
            ...
            self.last_comms[agent][int(actions[i]//self._n_actions)] = 1  # Normalize comms to [0, 1]. YOu can move up and have a one bit on each
        #print(self.agents)
#        new_comms = {}
...
#            self.last_comms[agent] = np.array(new_comms[agent], dtype=np.float32)
        ...
        print("Action dict:", action_dict)  # Debugging output
        ...
        print("terminated from env:", terminated)
        print("truncated from env:", truncated)
        print("dones:", dones)
        self.env.render()
        if self.t % 1 == 0:
            import pygame.surfarray
            if not hasattr(self, "frames"):
                self.frames = []
            # Access the underlying pygame Surface
            base_env = self.env
            if hasattr(base_env, "unwrapped"):
                print("Unwrapping base_env...")
                print(self.t, "t")
                base_env = base_env.unwrapped
            if hasattr(base_env, "graphic_pipeline"):
                print("Capturing frame...")
                surface = base_env.graphic_pipeline.screen
        ...
        self.last_obs = obs
        self.t += 1
        terminated = all(terminated) or all(truncated) or self.t >= self.episode_limit
        print(self.t, "t")
        print(self.episode_limit, "episode_limit")
        print(terminated, "terminated")
        print ("Rewards", sum(rewards.values()))
        print("Rewards,", rewards)
        print("Actions:", action_dict)
        return (
            [self._build_obs_player0(), self._build_obs_player1()],
            sum(rewards.values()),
            [dones[agent] for agent in self.agents],  # per-agent termination flags
            [truncated[agent] for agent in self.agents],  # per-agent truncated flags
            {}
        )
    ...
    def close(self):
#        if self.frames:
#            writer = imageio.get_writer("output.mp4", fps=30, codec='libx264')
